...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags ) {
    ...
    PushStyleVar( ImGuiStyleVar_FrameBorderSize, ImLerp( 0.0f, style.FrameBorderSize * 4.0f, animations.hover_fraction_outer ) );
    RenderFrame( bb.Min, bb.Max, hover_color_inner, animations.hover_fraction_outer > 0.0f, style.FrameRounding );
    PopStyleVar( );
    PopStyleColor( );
...
bool ImGui::BeginCombo( const char* label, int* v, const char* preview_value, ImGuiComboFlags flags ) {
    ...
    if ( label_size.x > 0 ) {
        ...
        frame_bb.Min.y += label_size.y + style.FramePadding.y * 2.0f;
        frame_bb.Max.y += label_size.y + style.FramePadding.y * 2.0f;
    }
    ...
    SetCursorPosY( GetCursorPosY( ) + (frame_bb.Max.y - frame_bb.Min.y) + style.FramePadding.y );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    const ImVec2 label_size = CalcTextSize( label, NULL, true );
    const ImRect frame_bb( window->DC.CursorPos + ImVec2( 0.0f, label_size.y + style.FramePadding.y * 2.0f ), window->DC.CursorPos + ImVec2( w, label_size.y + style.FramePadding.y * 2.0f + label_size.y * 0.5f ) );
    const ImRect total_bb( frame_bb.Min, frame_bb.Max + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f ) );
    ...
    if ( label_size.x > 0.0f )
        RenderText( ImVec2( frame_bb.Min.x, frame_bb.Min.y - (label_size.y + style.FramePadding.y * 2.0f) ), label );
    ...
    SetCursorPosY( GetCursorPosY( ) + (frame_bb.Max.y - frame_bb.Min.y) + style.FramePadding.y * 5.0f );
